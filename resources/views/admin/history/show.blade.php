...
@section('title', 'Detail Riwayat Skrining')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-[#001B48]">Detail Skrining</h1>
            <p class="text-sm text-gray-500 mt-1">Detail hasil diagnosa pengguna.</p>
        </div>
        <div class="flex gap-3">
            <a href="{{ route('admin.history.pdf', ['id' => $screening->id, 'action' => 'view']) }}" target="_blank" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-bold rounded-lg text-gray-700 bg-white hover:bg-gray-50 transition">
                <i data-lucide="file-text" class="w-4 h-4 mr-2 text-[#E3943B]"></i> Lihat PDF
            </a>
...
<!-- Container Utama -->
<div class="bg-gray-50 p-4 overflow-hidden flex flex-col">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full overflow-hidden">
        ...
        <div class="lg:col-span-3 flex flex-col gap-4 h-full overflow-y-auto custom-scrollbar pr-1">
            ...
            <div class="bg-white rounded-2xl shadow-sm border {{ $borderClass }} p-6 text-center flex flex-col items-center justify-center flex-shrink-0 relative overflow-hidden">
                ...
                <p class="text-xs text-gray-400 mt-2">{{ $screening->created_at->format('d M Y H:i') }}</p>
            </div>
